...
let write_re = (~bundled=false, path, component_list, colors) => {
  ...
  Printf.fprintf(
    oc,
    "module MuiTheme = { module Direction = { type t = | Ltr | Rtl; let toString = (direction: t) => switch direction { | Ltr => \"ltr\" | Rtl => \"rtl\" }; let fromString = (direction: string) => switch direction { | \"ltr\" => Ltr | \"rtl\" => Rtl | other => raise(Failure(\"Unknown direction: \" ++ other)) }; }; module Spacing = { type t = {unit: int}; }; module Transitions = { module Easing = { type t = { easeInOut: string, easeOut: string, easeIn: string, sharp: string }; }; module Duration = { type t = { shortest: int, shorter: int, short: int, standard: int, complex: int, enteringScreen: int, leavingScreen: int }; }; type t = { easing: Easing.t, duration: Duration.t, getAutoHeightDuration: int => int, create: (string, {. \"duration\": int}) => string /* create: (string, {. \"duration\": int, \"easing\": string, \"delay\": int}) => string */ }; }; type t = { direction: Direction.t, palette: unit, typography: unit, mixins: unit, breakpoints: unit, shadows: list(string), transitions: Transitions.t, spacing: Spacing.t, zIndex: int }; let fromJs = (theme) => { direction: Direction.fromString(theme##direction), palette: (), typography: (), mixins: (), breakpoints: (), shadows: Js.Array.reduce((lst, entry) => [entry, ...lst], [], theme##shadows), transitions: { easing: { easeInOut: theme##transitions##easing##easeInOut, easeOut: theme##transitions##easing##easeOut, easeIn: theme##transitions##easing##easeIn, sharp: theme##transitions##easing##sharp }, duration: { shortest: theme##transitions##duration##shortest, shorter: theme##transitions##duration##shorter, short: theme##transitions##duration##short, standard: theme##transitions##duration##standard, complex: theme##transitions##duration##complex, enteringScreen: theme##transitions##duration##enteringScreen, leavingScreen: theme##transitions##duration##leavingScreen }, getAutoHeightDuration: theme##transitions##getAutoHeightDuration, create: theme##transitions##create }, spacing: {unit: theme##spacing##unit}, zIndex: theme##zIndex }; };\n\n"
  );
  Printf.fprintf(
    oc,
    "module WithStyles = { let component = ReasonReact.statelessComponent(\"WithStyles\"); let make = (~render, ~classes: Js.t({..}), _children) => { ...component, render: (_self) => render(classes) }; type withStylesComponent('a) = [@bs] ('a => ReasonReact.reactClass); [@bs.module \"material-ui/styles\"] external withStylesExt : 'styles => withStylesComponent('component) = \"withStyles\"; let creteStylesWrapper = (styles) => withStylesExt(styles); let make = ( ~styles: option(Js.t({..}))=?, ~stylesWithTheme: option((MuiTheme.t => Js.t({..})))=?, ~render, children ) => ReasonReact.wrapJsForReason( ~reactClass={ let wrapper = creteStylesWrapper( switch styles { | Some(styles) => styles | None => switch stylesWithTheme { | Some(stylesWithTheme) => toJsUnsafe((theme) => stylesWithTheme(MuiTheme.fromJs(theme))) | None => Js.Obj.empty() } } ); [@bs] wrapper( ReasonReact.wrapReasonForJs( ~component, (jsProps) => make(~render=jsProps##render, ~classes=jsProps##classes, [||]) ) ) }, ~props={\"render\": render}, children ); };\n\n"
  );
...
let write_rei = (~bundled=false, path, component_list, colors) => {
  ...
  Printf.fprintf(
    oc,
    "module MuiTheme: { module Direction: { type t = | Ltr | Rtl; let toString: t => string; let fromString: string => t; }; module Spacing: { type t = {unit: int}; }; module Transitions: { module Easing: { type t = { easeInOut: string, easeOut: string, easeIn: string, sharp: string }; }; module Duration: { type t = { shortest: int, shorter: int, short: int, standard: int, complex: int, enteringScreen: int, leavingScreen: int }; }; type t = { easing: Easing.t, duration: Duration.t, getAutoHeightDuration: int => int, create: (string, {. \"duration\": int}) => string }; }; type t = { direction: Direction.t, palette: unit, typography: unit, mixins: unit, breakpoints: unit, shadows: list(string), transitions: Transitions.t, spacing: Spacing.t, zIndex: int }; let fromJs: theme => t; };\n\n"
  );
  Printf.fprintf(
    oc,
    "module WithStyles: {let make: (~styles: Js.t({..  })=?, ~stylesWithTheme: option((MuiTheme.t => Js.t({..})))=?, ~render: 'a, array(ReasonReact.reactElement)) =>ReasonReact.component(ReasonReact.stateless,  ReasonReact.noRetainedProps,  ReasonReact.actionless);};\n\n"
  );
